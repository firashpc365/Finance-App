...
const FluidCard: React.FC<FluidCardProps> = ({ title, value, sub, icon: Icon, color, onClick, children, actions, className = "" }) => {
  ...
  // This is a simplification; in production, you might map specific color props to full Tailwind classes.
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className={`relative group overflow-hidden rounded-[2rem] bg-slate-950/40 backdrop-blur-xl border border-white/5 hover:border-${colorBase}-500/30 transition-all duration-500 hover:shadow-2xl cursor-pointer h-full flex flex-col ${className}`}
    >
      ...
      <div className="relative z-10 p-8 flex flex-col h-full">
         <div className="flex justify-between items-start mb-6">
            <div className={`p-3.5 rounded-xl bg-white/5 border border-white/5 ${color} group-hover:text-white group-hover:bg-${colorBase}-500 transition-all duration-300 backdrop-blur-md shadow-[0_0_15px_rgba(0,0,0,0.2)]`}>
               <Icon size={24} />
            </div>
            {actions ? actions : (
      ...
      {/* Border Ring */}
      <div className="absolute inset-0 border border-white/10 rounded-[2rem] group-hover:border-white/20 transition-colors pointer-events-none ring-1 ring-inset ring-white/5" />
    </motion.div>
  );
};
