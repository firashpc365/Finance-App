...
const QuoteGenerator: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    setResult(null);
    try {
      const data = await generateQuote(client, items, commission, margin, deepAnalysis);
      setResult(data);
    } catch (err) {
      console.error(err);
              ...
              <div className="mt-12 pt-10 border-t-4 border-slate-900 break-inside-avoid">
                <div className="flex flex-col md:flex-row justify-between items-end gap-10">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed font-medium">{result.scope_of_work_text}</p>
                  </div>
